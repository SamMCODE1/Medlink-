...
-- Assign admin user to admin department if not already assigned
DO $$
DECLARE
  admin_user_id uuid;
  admin_dept_id uuid;
BEGIN
  ...
  SELECT user_id INTO admin_user_id FROM staff WHERE role = 'admin' LIMIT 1;
  
  -- Get the admin department ID
  SELECT id INTO admin_dept_id FROM departments WHERE name = 'Administration' LIMIT 1;
  
  -- If both exist, make the assignment
  IF admin_user_id IS NOT NULL AND admin_dept_id IS NOT NULL THEN
    -- Update the staff record to assign to admin department
    UPDATE staff 
    SET department_id = admin_dept_id
    WHERE user_id = admin_user_id AND (department_id IS NULL OR department_id != admin_dept_id);
  END IF;
END;
$$;
